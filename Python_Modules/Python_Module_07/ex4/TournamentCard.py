...
class TournamentCard(Card, Combatable, Rankable):
    ...
    def attack(self, target) -> Dict:
        damage = self.attack
        return {"attacker": self.name, "target": getattr(target, "name", str(target)), "damage": damage, "combat_resolved": True}

    def defend(self, incoming_damage: int) -> Dict:
        blocked = min(self.health, incoming_damage // 2)
        self.health -= incoming_damage - blocked
        still_alive = self.health > 0
        return {"defender": self.name, "damage_taken": incoming_damage, "damage_blocked": blocked, "still_alive": still_alive}
