...
class FantasyCardFactory(CardFactory):
    ...
    def create_creature(self, name_or_power) -> Card:
        ...
        return CreatureCard(name=name, cost=random.randint(1, 5),
                            rarity="Rare",
                            attack=random.randint(1, 7),
                            health=random.randint(1, 7))

    def create_spell(self, name_or_power) -> Card:
        return SpellCard(name=name_or_power,
                         cost=3, rarity="Common", effect_type="damage")

    def create_artifact(self, name_or_power) -> Card:
        return ArtifactCard(name=name_or_power,
                            cost=2, rarity="Uncommon",
    ...
    def create_themed_deck(self, size: int) -> Dict:
        ...
        for _ in range(size):
            deck.append(self.create_creature(
                random.choice(self.creature_types)))
            deck.append(self.create_spell(
                random.choice(self.spell_types)))
            deck.append(self.create_artifact(
                random.choice(self.artifact_types)))
        return {"deck": deck}
