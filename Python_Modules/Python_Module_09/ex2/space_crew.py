...
class SpaceMission(BaseModel):
    ...
    def validate_mission(cls, values):
        ...
        if not any(member.rank in {"commander", "captain"} for member in crew):
        ...
        if duration > 365:
            experienced_count = sum(1 for member
                                    in crew if member.years_experience >= 5)
            if experienced_count < len(crew)/2:
                raise ValueError("Long missions (>365 days)"
                                 "require 50% experienced crew (5+ years)")

        # All crew members must be active
        if not all(member.is_active for member in crew):
...
def main():
    ...
    try:
        ...
        return invalid_mission is None
    except ValidationError as e:
