...
def get_config():
    ...
    if config['API_KEY'] is None:
        print("[WARNING] API_KEY is not set! "
              "Services may not authenticate correctly.")
    if "secret" in config['DATABASE'
                          '_URL'] or "password" in config['DATABASE_URL']:
...
def main():
    ...
    print(f"API Access: {('Authenti'
                         'cated' if config['API_KEY'] else
                          'Unauthenticated')}")
    ...
    for key, value in config.items():
        if value and ("secret" in value.lower() or "pas"
                      "sword" in value.lower()):
