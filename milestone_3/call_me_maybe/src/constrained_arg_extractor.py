"""
Constrained Argument Extractor - Selezione tra Candidati

Approccio: Stesso principio della selezione delle funzioni.
1. Estrai CANDIDATI dal prompt (possibili valori)
2. Costruisci MATRICE di token per ogni candidato  
3. L'LLM SCEGLIE tra i candidati con constrained decoding
"""
...
class ConstrainedArgumentExtractor:
    """
    Estrae argomenti usando constrained decoding per SELEZIONARE
    tra candidati pre-estratti dal prompt.
    """
    ...
    def extract_arguments(
        ...
        for arg_name in arg_names:
            ...
            # Estrai candidati
            candidates = self._extract_candidates(prompt, fn_name, arg_name, arg_type, used_candidates)
            ...
            if len(candidates) == 1:
                ...
                # Usa constrained decoding per scegliere
                value = self._select_candidate(prompt, fn_name, arg_name, arg_type, candidates)
    ...
    def _extract_candidates(
        ...
        """Estrae candidati per un argomento."""
        candidates = []
        ...
        if "int" in arg_type or "float" in arg_type or "number" in arg_type:
            ...
            if fn_name == "fn_substitute_string_with_regex":
                candidates = self._extract_substitute_candidates(prompt, arg_name, used_candidates)
                ...
                candidates.extend([q for q in all_quoted if q not in used_candidates])
    ...
    def _extract_substitute_candidates(
        self,
        prompt: str,
        arg_name: str,
        used_candidates: List[str]
        ...
        if arg_name == "source_string":
            # CASO 1: "in 'X' with Y" o "in the string 'X' with Y"
            with_match = re.search(r"'\s+with\s+", prompt_lower)
            in_match = re.search(r"in\s+(?:the\s+)?(?:string\s+)?'", prompt_lower)
            
            if in_match and with_match and in_match.end() <= with_match.start():
            ...
            # CASO 2: "with 'Y' in 'X'" - source_string alla fine
            if not candidates:
                match = re.search(r"with\s+'[^']+'\s+in\s+'(.+)'(?:\s*$|\s+\w)", prompt, re.IGNORECASE)
                if match:
                    candidates.append(match.group(1))
            
            # CASO 3: fallback - stringa dopo "in '" fino alla fine
            if not candidates:
                match = re.search(r"in\s+'([^']+)'(?:\s*$)", prompt, re.IGNORECASE)
                if match:
                    candidates.append(match.group(1))
            ...
            if "consonant" in prompt_lower:
                candidates.append("[bcdfghjklmnpqrstvwxyzBCDFGHJKLMNPQRSTVWXYZ]")
            if "space" in prompt_lower or "whitespace" in prompt_lower:
                candidates.append(r"\s+")
            
            # "word 'X'"
            word_match = re.search(r"(?:the\s+)?word\s+['\"]?(\w+)['\"]?", prompt, re.IGNORECASE)
            if word_match:
                candidates.append(word_match.group(1))
        
        elif arg_name == "replacement":
            # CASO 1: "with 'X' in" (replacement prima di source_string)
            match = re.search(r"with\s+'([^']+)'\s+in\s+", prompt, re.IGNORECASE)
            if match:
                candidates.append(match.group(1))
            
            # CASO 2: "with 'X'" alla fine
            if not candidates:
                match = re.search(r"with\s+'([^']+)'\s*$", prompt, re.IGNORECASE)
                if match:
                    candidates.append(match.group(1))
            
            # CASO 3: "with X" (parola semantica alla fine)
            if not candidates:
                match = re.search(r"with\s+(\w+)\s*$", prompt, re.IGNORECASE)
                if match:
                    word = match.group(1).lower()
                    semantic_map = {
                        "asterisks": "*", "asterisk": "*", "stars": "*", "star": "*",
    ...
    def _select_candidate(
        ...
        """
        Usa CONSTRAINED DECODING per selezionare tra candidati.
        ...
        # Costruisci prompt
        selection_prompt = self._build_selection_prompt(prompt, fn_name, arg_name, arg_type, candidates)
        input_ids = self.tokenizer.encode(selection_prompt, add_special_tokens=False)
        
        # Costruisci matrice di token per ogni candidato
        candidate_matrix = []
        for candidate in candidates:
            tokens = self.tokenizer.encode(str(candidate), add_special_tokens=False)
            candidate_matrix.append(tokens)
        
        # Genera con constrained decoding
        ...
        for _ in range(max_tokens):
            ...
            # Token permessi
            allowed_ids = self._get_allowed_tokens(generated_ids, candidate_matrix)
            if not allowed_ids:
                break
            ...
            # Check match univoco
            is_unique, matched = self._check_unique_match(generated_ids, candidate_matrix, candidates)
            if is_unique:
                return matched
        
        return self._find_best_match(generated_ids, candidate_matrix, candidates)
...
Answer: """
    ...
    def _get_allowed_tokens(
        ...
        """Token che continuano almeno un candidato."""
        allowed = set()
